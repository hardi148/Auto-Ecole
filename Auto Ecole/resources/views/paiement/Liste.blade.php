    ...
    <div class="d-flex flex-column" id="content-wrapper">
      <div id="content">
        ...
        <div class="container-fluid">
            <form action="{{ url('/recherchepaiementecolage') }}" method="post">
              {{ csrf_field() }}  
...
@endif
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td class="border-bottom-0"> 
                        {{ FormatNumber::formatter($rows->montant) }} Ar
                      </td>
                      <td class="border-bottom-0">
                        {{ $rows->typepermi }}
                      </td>
  ...
  <a class="page-link" href="{{ $currentPage == 1 ? '#' : url('/paginationpaiementecolage') }}/{{ $currentPage - 1 }}" aria-label="Précédent">
  ...
  <a class="page-link" href="{{ url('/paginationpaiementecolage') }}/{{ $rows }}">{{ $rows }}</a>
  </li>
  @endforeach
  <li class="page-item {{ $currentPage == $lastPage ? 'disabled' : '' }}">
  <a class="page-link" href="{{ $currentPage == $lastPage ? '#' : url('/paginationpaiementecolage') }}/{{ $currentPage + 1 }}" aria-label="Suivant">
