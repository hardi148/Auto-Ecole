    ...
    <div class="d-flex flex-column" id="content-wrapper">
      <div id="content">
        ...
        <div class="container-fluid">
            <form action="{{ url('/rechercheexamencode') }}" method="post">
              {{ csrf_field() }}  
...
@endif
                ...
                <table class="table text-nowrap mb-0 align-middle">
                  ...
                  <tbody>
                    ...
                    <tr>
                      ...
                      @if($rows->resultat == 0)
                      <td class="border-bottom-0">
                        <a class="btn btn-primary" href="{{ url('/annulercode') }}/{{ $rows->idexamen }}" style="background: var(--bs-gray);border-style: none;margin-top: 5px;">Annuler</a>
                      </td>
                      <td class="border-bottom-0">
                        <a class="btn btn-primary" href="{{ url('/emettrecode') }}/{{ $rows->idexamen }}" style="background: var(--bs-gray);border-style: none;margin-top: 5px;">Emettre un resultat</a>
                      </td>
                      @endif
                    </tr>          
                    @endforeach           
  ...
  <a class="page-link" href="{{ $currentPage == 1 ? '#' : url('/paginationexamencode') }}/{{ $currentPage - 1 }}" aria-label="Précédent">
  ...
  <a class="page-link" href="{{ url('/paginationexamencode') }}/{{ $rows }}">{{ $rows }}</a>
  </li>
  @endforeach
  <li class="page-item {{ $currentPage == $lastPage ? 'disabled' : '' }}">
  <a class="page-link" href="{{ $currentPage == $lastPage ? '#' : url('/paginationexamencode') }}/{{ $currentPage + 1 }}" aria-label="Suivant">
